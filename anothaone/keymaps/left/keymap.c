...
enum layers{
    ...
    const uint16_t PROGMEM bqlsft[] = {KC_SPC,KC_O,COMBO_END};

    const uint16_t PROGMEM blsft[] = {KC_SPC,KC_O,KC_N,COMBO_END};
    const uint16_t PROGMEM blgui[] = {KC_SPC,KC_O,KC_A,COMBO_END};
    const uint16_t PROGMEM bralt[] = {KC_SPC,KC_O,KC_H,COMBO_END};
    const uint16_t PROGMEM blctl[] = {KC_SPC,KC_O,KC_E,COMBO_END};

    const uint16_t PROGMEM blsftlgui[] = {KC_SPC,KC_O,KC_N,KC_A,COMBO_END};
    const uint16_t PROGMEM blsftralt[] = {KC_SPC,KC_O,KC_N,KC_H,COMBO_END};
    const uint16_t PROGMEM blsftlctl[] = {KC_SPC,KC_O,KC_N,KC_E,COMBO_END};
    const uint16_t PROGMEM blguiralt[] = {KC_SPC,KC_O,KC_A,KC_H,COMBO_END};
    const uint16_t PROGMEM blguilctl[] = {KC_SPC,KC_O,KC_A,KC_E,COMBO_END};
    const uint16_t PROGMEM braltlctl[] = {KC_SPC,KC_O,KC_H,KC_E,COMBO_END};

    const uint16_t PROGMEM btab[] = {KC_SPC,KC_A,COMBO_END};
    const uint16_t PROGMEM bent[] = {KC_SPC,KC_E,COMBO_END};
    ...
    const uint16_t PROGMEM slsft[] = {KC_SPC,KC_SCLN,KC_LCBR,COMBO_END};
    const uint16_t PROGMEM slgui[] = {KC_SPC,KC_SCLN,KC_LBRC,COMBO_END};
    const uint16_t PROGMEM sralt[] = {KC_SPC,KC_SCLN,KC_RCBR,COMBO_END};
    const uint16_t PROGMEM slctl[] = {KC_SPC,KC_SCLN,KC_RBRC,COMBO_END};

    const uint16_t PROGMEM slsftlgui[] = {KC_SPC,KC_SCLN,KC_LCBR,KC_LBRC,COMBO_END};
    const uint16_t PROGMEM slsftralt[] = {KC_SPC,KC_SCLN,KC_LCBR,KC_RCBR,COMBO_END};
    const uint16_t PROGMEM slsftlctl[] = {KC_SPC,KC_SCLN,KC_LCBR,KC_RBRC,COMBO_END};
    const uint16_t PROGMEM slguiralt[] = {KC_SPC,KC_SCLN,KC_LBRC,KC_RCBR,COMBO_END};
    const uint16_t PROGMEM slguilctl[] = {KC_SPC,KC_SCLN,KC_LBRC,KC_RBRC,COMBO_END};
    const uint16_t PROGMEM sraltlctl[] = {KC_SPC,KC_SCLN,KC_RCBR,KC_RBRC,COMBO_END};

    const uint16_t PROGMEM stab[] = {KC_SPC,KC_LBRC,COMBO_END};
    const uint16_t PROGMEM sent[] = {KC_SPC,KC_RBRC,COMBO_END};
    ...
    // BSPC SPC
    const uint16_t PROGMEM fthree[] = {LT(NAV,KC_BSPC),KC_SPC,KC_A,COMBO_END};
    const uint16_t PROGMEM ffour[] = {LT(NAV,KC_BSPC),KC_SPC,KC_E,COMBO_END};
    const uint16_t PROGMEM fone[] = {LT(NAV,KC_BSPC),KC_SPC,KC_N,COMBO_END};
    const uint16_t PROGMEM ftwo[] = {LT(NAV,KC_BSPC),KC_SPC,KC_H,COMBO_END};
    const uint16_t PROGMEM pscr[] = {LT(NAV,KC_BSPC),KC_SPC,KC_O,COMBO_END};

    const uint16_t PROGMEM ffive[] = {LT(NAV,KC_BSPC),KC_SPC,KC_O,KC_N,COMBO_END};
    const uint16_t PROGMEM fsix[] = {LT(NAV,KC_BSPC),KC_SPC,KC_O,KC_H,COMBO_END};
    const uint16_t PROGMEM fseven[] = {LT(NAV,KC_BSPC),KC_SPC,KC_O,KC_A,COMBO_END};
    const uint16_t PROGMEM feight[] = {LT(NAV,KC_BSPC),KC_SPC,KC_O,KC_E,COMBO_END};
    const uint16_t PROGMEM fnine[] = {LT(NAV,KC_BSPC),KC_SPC,KC_N,KC_A,COMBO_END};
    const uint16_t PROGMEM ften[] = {LT(NAV,KC_BSPC),KC_SPC,KC_H,KC_E,COMBO_END};
    const uint16_t PROGMEM feleven[] = {LT(NAV,KC_BSPC),KC_SPC,KC_O,KC_N,KC_A,COMBO_END};
    const uint16_t PROGMEM ftwelve[] = {LT(NAV,KC_BSPC),KC_SPC,KC_O,KC_H,KC_E,COMBO_END};
    const uint16_t PROGMEM briu[] = {LT(NAV,KC_BSPC),KC_SPC,KC_H,KC_A,COMBO_END};
    const uint16_t PROGMEM brid[] = {LT(NAV,KC_BSPC),KC_SPC,KC_N,KC_E,COMBO_END};
    ...
    const uint16_t PROGMEM nlsft[] = {KC_SPC,KC_DEL,KC_UP,COMBO_END};
    const uint16_t PROGMEM nlgui[] = {KC_SPC,KC_DEL,KC_LEFT,COMBO_END};
    const uint16_t PROGMEM nralt[] = {KC_SPC,KC_DEL,KC_DOWN,COMBO_END};
    const uint16_t PROGMEM nlctl[] = {KC_SPC,KC_DEL,KC_RGHT,COMBO_END};

    const uint16_t PROGMEM nlsftlgui[] = {KC_SPC,KC_DEL,KC_UP,KC_LEFT,COMBO_END};
    const uint16_t PROGMEM nlsftralt[] = {KC_SPC,KC_DEL,KC_UP,KC_DOWN,COMBO_END};
    const uint16_t PROGMEM nlsftlctl[] = {KC_SPC,KC_DEL,KC_UP,KC_RGHT,COMBO_END};
    const uint16_t PROGMEM nlguiralt[] = {KC_SPC,KC_DEL,KC_LEFT,KC_DOWN,COMBO_END};
    const uint16_t PROGMEM nlguilctl[] = {KC_SPC,KC_DEL,KC_LEFT,KC_RGHT,COMBO_END};
    const uint16_t PROGMEM nraltlctl[] = {KC_SPC,KC_DEL,KC_DOWN,KC_RGHT,COMBO_END};

    const uint16_t PROGMEM ntab[] = {KC_SPC,KC_LEFT,COMBO_END};
    const uint16_t PROGMEM nent[] = {KC_SPC,KC_RGHT,COMBO_END};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [BASE] = LAYOUT_3x6_3(
                        KC_N,   KC_A,
        KC_SPC, KC_O,   KC_H,   KC_E,
        LT(NAV,KC_BSPC),LT(SYM,KC_T)
    ),
    [SYM] = LAYOUT_3x6_3(
                        KC_LCBR,KC_LBRC,
        KC_SPC, KC_SCLN,KC_RCBR,KC_RBRC,
                        KC_NO,  KC_NO
    ),
    [NAV] = LAYOUT_3x6_3(
                        KC_UP,  KC_LEFT,
        KC_SPC, KC_DEL, KC_DOWN,KC_RGHT,
                        KC_NO,  KC_NO
    )
};
